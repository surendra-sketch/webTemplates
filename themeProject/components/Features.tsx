...
const Features: React.FC = () => {
  return (
    <section id="features" className="py-32 bg-background relative transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          ...
          >
            Powering the <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Future of Work</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="bg-card/50 backdrop-blur-sm border border-white/5 rounded-3xl p-8 hover:bg-card hover:border-primary/20 transition-all group shadow-lg"
            >
              <div className="w-14 h-14 rounded-2xl bg-background border border-white/10 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-transform shadow-inner">
                {feature.icon}
              </div>
